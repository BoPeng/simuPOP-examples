...
class pedProbabilities:
    ...
    def _getHaploFreq(self):
        ...
        ** STEP 2 in documentation. **
        ...
        if self.h["AX"] - self.x * self.p <= 0:
            raise ValueError(
                "h11-xp can NOT be negative, adjust p and x to restrict it to be positive"
            )
    ...
    def _getWholeGenoProbs(self):
        ...
        ** Part of STEP 3 in the documentation **
        ...
        for key in itertools.product(
            *tuple([("AA", "AB", "BB") for x in self.pedType])
        ):
            if key[:2] == ("AA", "AA"):
    ...
    def PrMarkerGivenPed(self, geno):
        ...
        ** STEP 5 in the documentation **
        ...
        if geno == "XX":
            return (
                self.PrGenoGivenPed("AA") * (self.h["AX"] ** 2) / self.PrGeno("AA")
                + 2
                * self.PrGenoGivenPed("AB")
                * self.h["AX"]
                * self.h["BX"]
                / self.PrGeno("AB")
            ...
            return (
                2
                * self.PrGenoGivenPed("AA")
                * self.h["AX"]
                * self.h["AY"]
                / self.PrGeno("AA")
                + 2
                * self.PrGenoGivenPed("AB")
                * (self.h["AX"] * self.h["BY"] + self.h["BX"] * self.h["AY"])
                / self.PrGeno("AB")
                + 2
                * self.PrGenoGivenPed("BB")
                * self.h["BX"]
                * self.h["BY"]
                / self.PrGeno("BB")
            )
            ...
            return (
                self.PrGenoGivenPed("AA") * (self.h["AY"] ** 2) / self.PrGeno("AA")
                + 2
                * self.PrGenoGivenPed("AB")
                * self.h["AY"]
                * self.h["BY"]
    ...
    def PrAlleleGivenPed(self, allele):
        ...
        ** STEP 6 in the documentation **
        """
        if allele == "X":
            return self.PrMarkerGivenPed("XX") + self.PrMarkerGivenPed("XY") / 2.0
        else:
            return self.PrMarkerGivenPed("YY") + self.PrMarkerGivenPed("XY") / 2.0
...
class powerCalculator:
    ...
    def __init__(
        self, mode, K, p=0, x=0, Dprime=1, R2=1, genoP=0, genoX=0, logger=None
    ):
        """
        Parameters description:
    ...
    def expectedFreq(self, samples):
        ...
        """
        allP = 0
        for ped, cnt in samples.items():
            # change aa to AA.
            probs = pedProbabilities(
                ped.upper(), self.p, self.r, self.x, self.Dprime, self.R2
            )
    ...
    def getPower(self, Grr, cases, controls, alpha):
        ...
        if len(controls) == 0:
            raise ValueError(
                "Number of controls should be given to calculate statistical power"
            )
        if len(cases) == 0:
            raise ValueError(
                "Number of cases should be given to calculate statistical power"
            )
        #
        ...
        v1 = (
            gsl_cdf_ugaussian_Pinv(alpha / 2.0) * sigmaH0
            + ((2 * N2) ** 0.5) * (p2 - p1)
        ) / sigmaH1
        v2 = (
            gsl_cdf_ugaussian_Pinv(alpha / 2.0) * sigmaH0
        ...
        if self.logger:
            self.logger.info(
                "p ctrl: %.4f p case: %.4f sigma_H1: %.7f statistic: %.4f power: %.4f"
                % (
                    p1,
                    p2,
                    sigmaH1 / ((2 * N2) ** 0.5),
                    ((2 * N2) ** 0.5) * (p1 - p2) / sigmaH1,
                    power,
                )
            )
        return power
    ...
    def getRelativeRisk(self, cases, controls, alpha, power):
        ...
        while True:
            if maxGrr - minGrr < 1e-7:
                if self.mode == "Additive_AA":
                    Grr = (
                        Grr * self.p * self.p + (Grr + 1.0) * self.p * (1 - self.p)
                    ) / (self.p * self.p + 2 * self.p * (1.0 - self.p))
                return Grr
            p = self.getPower(Grr, cases, controls, alpha)
            if p > power:
                maxGrr = Grr
                Grr = (minGrr + maxGrr) / 2.0
            elif p <= power:
                minGrr = Grr
                Grr = (minGrr + maxGrr) / 2.0
            if self.logger:
                self.logger.info(
                    "Grr: %.4f (min: %.4f, max: %.4f), Power: %.4f"
                    % (Grr, minGrr, maxGrr, p)
                )
        return 0
    ...
    def getSampleSizeFromRatio(self, Grr, cases, controls, ratio, alpha, power):
        ...
        N2 = (
            (
                (
                    gsl_cdf_ugaussian_Pinv(alpha / 2) * sigmaH1
                    + gsl_cdf_ugaussian_Pinv(1 - power) * sigmaH1
                )
                / (p2 - p1)
            )
            ** 2
    ...
    def getSampleSizeFromControls(self, Grr, cases, controls, alpha, power):
        ...
        while True:
            ...
            if pp < power:
                ...
                if N2_upper >= 1e10:
                    raise ValueError(
                        "Specified power is not achievable. Please adjust disease model or number of controls"
                    )
            else:
                break
        # bisection method
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="""Mode of inheritance
        Disease models with different relationship between risks of AA, AB and BB,
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "Sample size from fixed #ctrl",
            "Sample size from fixed #case",
        ],
        help="""Type of analysis
        Perform different types of calculations, each require the specification of
    ...
    if pars.analysis == "Statistical power":
        ...
        for K, p, x, genoP, genoX, alpha, Grr in itertools.product(
            ...
            print(
                ...
                % (
                    K,
                    p if genoP == 0.0 else genoP,
                    x if genoX == 0.0 else genoX,
                    pars.Dprime if pars.Dprime == 1 else pars.R2,
                    alpha,
                    Grr,
                    powerCalculator(
                        mode=pars.mode,
            ...
            print(
                ...
                % (
                    ...
                    powerCalculator(
                        ...
                        genoP=genoP,
                        genoX=genoX,
                    ).getSampleSizeFromRatio(
                        Grr=Grr,
                        cases=pars.cases,
                        controls=pars.controls,
                        ratio=pars.ratio,
                        alpha=alpha,
                        power=power,
                    ),
