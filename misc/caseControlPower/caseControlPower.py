...
class powerCalculator:
    ...
    def getPower(self, Grr, cases, controls, alpha):
        ...
        v1 = (gsl_cdf_ugaussian_Pinv(alpha/2.)*sigmaH0 + ((2*N2)**0.5)*(p2-p1)) / sigmaH1
        v2 = (gsl_cdf_ugaussian_Pinv(alpha/2.)*sigmaH0 - ((2*N2)**0.5)*(p2-p1)) / sigmaH1
        ...
        while True:
            if maxGrr - minGrr < 1e-7:
                if self.mode == 'Additive_AA':
                    Grr = (Grr*self.p*self.p + (Grr+1.)*self.p*(1-self.p)) / (self.p*self.p + 2*self.p*(1.-self.p))
                return Grr
...
if __name__ == '__main__':
    ...
    if pars.analysis == 'Statistical power':
        ...
        for K, p, x, genoP, genoX, alpha, power in itertools.product(
            ...
            grr = powerCalculator(mode=pars.mode, K=K, p =p, x=x,
